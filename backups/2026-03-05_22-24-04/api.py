...
# Criar tabelas no banco (não impede a API de subir se o MySQL estiver offline)
try:
    Base.metadata.create_all(bind=engine)
except Exception as e:
    print(f"[AVISO] Banco não disponível na subida: {e}. API sobe na porta 8000; conecte o MySQL e reinicie para dados completos.")
...
def on_startup():
    """Ao subir a API, garante que existam formas de pagamento e plano de contas no banco (se o banco estiver acessível)."""
    try:
...
class ModeloVeiculoResponse(BaseModel):
    ...
    id_marca: int
    nome: str
    marca_nome: Optional[str] = None  # preenchido quando listar com join
    class Config:
        from_attributes = True
...
class VeiculoResponse(BaseModel):
    ...
    placa: str
    id_modelo: int
    marca: str = ""   # nome da marca (JSON)
    modelo: str = ""  # nome do modelo (JSON)
    ano: int
    cor: str
    quilometragem: float
    valor_diaria: float
    ...
    def orm_marca_modelo(cls, data):
        """Preenche marca/modelo a partir de marca_nome/modelo_nome quando o input é ORM."""
    ...
    def from_veiculo_orm(cls, v):
        """Constrói response com marca e modelo a partir da relação."""
        return cls(
            id=v.id,
            placa=v.placa,
...
@app.get("/marcas/", response_model=List[MarcaVeiculoResponse])
def listar_marcas(db: Session = Depends(get_db)):
    """Lista todas as marcas de veículos."""
    return db.query(models.MarcaVeiculo).order_by(models.MarcaVeiculo.nome).all()

@app.post("/marcas/", response_model=MarcaVeiculoResponse, status_code=status.HTTP_201_CREATED)
...
@app.get("/modelos/", response_model=List[ModeloVeiculoResponse])
def listar_modelos(id_marca: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    query = db.query(models.ModeloVeiculo).options(
        joinedload(models.ModeloVeiculo.marca)
    )
...
@app.post("/modelos/", response_model=ModeloVeiculoResponse, status_code=status.HTTP_201_CREATED)
...
def _query_veiculos_com_marca_modelo(db):
...
def atualizar_veiculo(veiculo_id: int, veiculo_update: VeiculoUpdate, db: Session = Depends(get_db)):
    ...
    dados_atualizacao = veiculo_update.model_dump(exclude_unset=True) if hasattr(veiculo_update, 'model_dump') else veiculo_update.dict(exclude_unset=True)

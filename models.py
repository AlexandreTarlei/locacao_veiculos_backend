...
class Reserva(Base):
    """Reserva de veículo (período reservado antes da locação efetiva)."""
    __tablename__ = "reservas"

    id = Column(Integer, primary_key=True, index=True)
    empresa_id = Column(Integer, nullable=True, index=True)  # opcional; FK para empresas quando existir
    ...
    def __str__(self):
        return f"[ID: {self.id}] Cliente #{self.cliente_id} - Veículo #{self.veiculo_id} - {self.data_inicio} a {self.data_fim} ({self.status})"
...
class Contrato(Base):
    ...
    data_contrato = Column(DateTime, default=datetime.now, nullable=False)
    ...
    def __str__(self):
        return f"[ID: {self.id}] Cliente #{self.cliente_id} - Veículo #{self.veiculo_id} - R$ {self.valor_total or 0:.2f} ({self.status})"


class Manutencao(Base):
    """Registros de manutenção/gastos por veículo."""
...
class LancamentoFinanceiro(Base):
    ...
    id_plano_conta = Column("id_plano_conta", Integer, nullable=False)
    id_forma_pagamento = Column("id_forma_pagamento", Integer, ForeignKey("formas_pagamento.id"), nullable=False)
